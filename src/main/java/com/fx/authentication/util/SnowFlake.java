package com.fx.authentication.util;
...
public class SnowFlake {
    ...
    /**
     * 雪花算法生成器，需要配置文件中添加{@code snow-flake.data-center-id }配置；<p>
     * 如果不需要此生成器，可以添加{@code snow-flake.enable=false }，禁用此Bean配置
     *
     * @param dataCenterId 生成雪花ID需要的数据中心配置，取值范围：0-31
     * @return 雪花算法生成器
     */
    @Bean
    @ConditionalOnProperty(name = "enable", prefix = "snow-flake", havingValue = "true", matchIfMissing = true)
    public SnowFlake snowFlake(@Value("${snow-flake.data-center-id}") long dataCenterId) {
        return new SnowFlake(dataCenterId);
    }
}
